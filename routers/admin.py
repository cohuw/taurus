...
from richfmt import EM_T, EM_TC, heading, para, table, send_rich, send_rich_to
import datetime
...
async def take_currency(message: Message, economy: EconomyService, settings: Settings, currency: str) -> None:
    ...
    try:
        ...
        return
    curr_str = f'{EM_T} T' if currency == 'T' else f'{EM_TC} TC'
    await send_rich(message, [para(f'<b>Успешно изъято {amount} {curr_str}</b> у пользователя <code>{target_id}</code>.')])
...
async def admin_save_tg_price(message: Message, economy: EconomyService, state: FSMContext) -> None:
    if message.text and message.text.isdigit():
    ...
    await state.clear()
    rate = await economy.get_rate()
    await send_rich(message, [heading('Успех'), para(f'<b>Курс обновлён:</b> 1 {EM_T} T = {rate} {EM_TC} TC')])
...
async def give_prizes_scope(callback: CallbackQuery, economy: EconomyService, state: FSMContext) -> None:
    ...
    if callback.message:
        await send_rich(callback.message, [para(f'Начислено <b>{amount} {curr_str}</b> [{scope}]: успешно {ok}, ошибок {fail}.')])
